...
mod integration_tests {
    ...
    fn deploy_mock_node(harness: &mut CalcTestApp, size_weight: u16) -> Addr {
        ...
        harness
            .app
            .wrap()
            .query_wasm_smart::<Vec<RegisteredNode>>(
                harness.manager_addr.clone(),
                &ManagerQueryMsg::Nodes {
                    start_after: None,
                    limit: None,
                },
            )
    ...
    fn test_instantiate_strategy_with_all_node_types_succeeds() {
        ...
        let fin_pair = harness.fin_addr.clone();
        ...
        let nodes = vec![
            ...
            Node::Condition {
                condition: Condition::StrategyStatus {
                    ...
                    contract_address: other_strategy.strategy_addr,
    ...
    fn test_execute_thor_swap_action_with_zero_balance_succeeds() {
        ...
        strategy.execute().assert_strategy_balances(&[]);
    }
    ...
    fn test_execute_fin_swap_action_with_swap_amount_scaled_to_zero_skips() {
        ...
        strategy
            .execute()
            .assert_strategy_balances(&[swap_action.swap_amount.clone()]);
    }
    ...
    fn test_execute_fin_swap_action_with_less_balance_than_swap_amount_executes() {
        ...
        strategy.execute().assert_strategy_balances(&[Coin::new(
    ...
    fn test_instantiate_swap_action_immediately_executes() {
        ...
        strategy.assert_strategy_balance(&Coin::new(
            swap_action
                .swap_amount
                .amount
                .mul_floor(Decimal::percent(99)),
    ...
    fn test_execute_swap_action_with_single_route_succeeds() {
        ...
        strategy.execute().assert_strategy_balance(&Coin::new(
            swap_route
                .swap_amount
                .amount
                .mul_floor(Decimal::percent(99))
    ...
    fn test_execute_limit_order_action_with_filled_ratio_less_than_min_fill_ratio_skips_claiming() {
        ...
        let order_action = FinLimitOrder {
            strategy: PriceStrategy::Fixed(Decimal::percent(50)),
            min_fill_ratio: Some(Decimal::percent(25)),
            ..default_limit_order_action(&harness)
        };
    ...
    fn test_execute_limit_order_action_with_filled_ratio_more_than_min_fill_ratio_claims() {
        ...
        let order_action = FinLimitOrder {
            strategy: PriceStrategy::Fixed(Decimal::percent(50)),
            min_fill_ratio: Some(Decimal::percent(15)),
            ..default_limit_order_action(&harness)
        };
    ...
    fn test_execute_limit_order_action_with_additional_balance_deploys_it() {
        let mut harness = CalcTestApp::setup();
        let order_action = FinLimitOrder {
            strategy: PriceStrategy::Fixed(Decimal::percent(1000)),
            ..default_limit_order_action(&harness)
        };
        let starting_balance = Coin::new(1_000u128, order_action.bid_denom.clone());
    ...
    #[test]
    fn test_execute_limit_order_action_with_empty_bid_denom_balance_sets_no_order() {
        let mut harness = CalcTestApp::setup();
        let order_action = FinLimitOrder {
            strategy: PriceStrategy::Fixed(Decimal::from_str("0.1").unwrap()),
            ..default_limit_order_action(&harness)
        };
        let starting_balance = Coin::new(1000u128, order_action.bid_denom.clone());

        let fin_pair = harness.query_fin_config(&order_action.pair_address);
        let ask_denom = fin_pair.denoms.ask(&order_action.side);
    ...
    fn test_execute_limit_order_action_with_new_desired_price_outside_tolerance_updates_order() {
        ...
        strategy
            ...
            .assert_strategy_fin_orders(
                ...
                vec![(
                    order_action.side.clone(),
                    Decimal::from_str("0.505").unwrap(), // price
                    starting_balance.amount,             // offer
                    starting_balance.amount,             // remaining
                    Uint128::zero(),                     // filled
                )],
        ...
        strategy
            ...
            .set_fin_orders(
                ...
                vec![(
                    Side::Base,
                    Price::Fixed(Decimal::from_str("0.9").unwrap()),
                    Some(new_order_amount.amount.clone()),
                )],
                &[new_order_amount],
            )
            .unwrap();
    ...
    fn test_execute_limit_order_action_with_new_desired_price_inside_tolerance_skips() {
        ...
        let order_action = FinLimitOrder {
            strategy: PriceStrategy::Offset {
                ...
                direction: Direction::Below,
                offset: Offset::Percent(50),
                tolerance: Some(Offset::Percent(10)),
            },
            pair_address: harness.fin_addr.clone(),
            side: Side::Quote,
            bid_denom: pair.denoms.quote().to_string(),
    ...
    fn test_instantiate_distribution_with_native_denom_and_non_deposit_recipients_succeeds() {
        ...
        strategy.assert_strategy_balance(&Coin::new(0u128, "x/ruji"));
    ...
    fn test_execute_condition_node_with_satisfied_conditions_executes_on_success() {
        ...
        StrategyBuilder::new(&mut harness)
            ...
            .instantiate(&funds)
            .assert_strategy_balance(&Coin::new(1u128, swap_action.swap_amount.denom.clone()));
    }
    ...
    fn test_execute_condition_node_respects_asset_value_ratio_condition() {
        ...
        strategy.assert_address_balances(&strategy_addr, &funds);
